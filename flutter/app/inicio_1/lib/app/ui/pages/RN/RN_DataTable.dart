...
class RNDataTable extends StatelessWidget {
      ...
      builder: (_) {
        return FutureBuilder(
          future: getProductDataSource(
              ...
              rango: int.parse(double.parse(_.week.text).toStringAsFixed(0)),
...
// Obtener Colum INICIO
  List<GridColumn> getColumns() {
    return <GridColumn>[
      ...
      repetirColumn(columnName: 'sd_de_3', nombreColumn: '+3 DE'),
    ];
  }
...
// Leer Data INICIO
  Future<List<SalesWeek>> generateProductList(
      {fileJson, rango, interpretacion}) async {
    var response = await rootBundle.loadString(fileJson);
    var decodedProducts = jsonDecode(response)
        .getRange(rango, rango + 1)
        .cast<Map<String, dynamic>>();
    List<SalesWeek> productList = await decodedProducts
        .map<SalesWeek>((json) => SalesWeek.fromJson(json))
        .toList();
...
class ProductDataGridSource extends DataGridSource {
  ...
  late List<SalesWeek> productList;
