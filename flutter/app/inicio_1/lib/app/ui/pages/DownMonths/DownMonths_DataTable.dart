...
class DownMonthsDataTable extends StatelessWidget {
  final String fileJsonData;
  final double interpretacion;
  const DownMonthsDataTable(
      ...
      builder: (_) {
        return FutureBuilder(
          future: getProductDataSource(
              ...
              rango:
                  int.parse((double.parse(_.mes.text) - 1).toStringAsFixed(0)),
          ...
          builder: (BuildContext context, AsyncSnapshot<dynamic> snapshot) {
            return snapshot.hasData
                ...
                : const Text(
                    'El valor de los meses de nacimiento debe estar entre 1 y 36 ');
          },
        );
      },
    );
...
// Obtener Colum INICIO
  List<GridColumn> getColumns() {
    return <GridColumn>[
      ...
      repetirColumn(columnName: 'mes', nombreColumn: 'Mes'),
...
// Leer Data INICIO
  Future<List<SalesDownMonth>> generateProductList(
      {fileJson, rango, interpretacion}) async {
    var response = await rootBundle.loadString(fileJson);
    var decodedProducts = jsonDecode(response)
        .getRange(rango, rango + 1)
        .cast<Map<String, dynamic>>();
    List<SalesDownMonth> productList = await decodedProducts
        .map<SalesDownMonth>((json) => SalesDownMonth.fromJson(json))
        .toList();
...
class ProductDataGridSource extends DataGridSource {
  ...
  late List<SalesDownMonth> productList;
  ...
  void buildDataGridRow(interpretacion) {
    dataGridRows = productList.map<DataGridRow>((dataGridRow) {
      ...
      return DataGridRow(cells: [
        ...
        DataGridCell<int>(columnName: 'mes', value: dataGridRow.months),
